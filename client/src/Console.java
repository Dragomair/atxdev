...
public class Console {
	...
	public void drawConsole(int width, int height) {
		if (openConsole) {
			...
			Client.newBoldFont.drawBasicString(consoleInput + (Client.loopCycle % 20 < 10 ? "|" : ""), 35, height - 6,
					0xffffff, 0);
			...
			if (inputConsoleMessages[0].length() <= 0) {
				sendConsoleMessage(
						"Type 'console_commands' for a list of commands or 'clear_console' to clear the console.",
						false);
			}
		}
	}
	...
	public void sendConsoleCommands(String consoleCommand) {
		if (consoleCommand.equals("clear_console")) {
			...
			sendConsoleMessage(
					"Type 'console_commands' for a list of commands or 'clear_console' to clear the console.", false);
		}
