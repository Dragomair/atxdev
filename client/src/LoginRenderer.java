...
public class LoginRenderer {
	...
	public void displayLoginScreen() {
		...
		if (client.loginScreenState == 0) {
			Client.cacheSprite[6].drawARGBSprite((Client.frameWidth / 2) - (Client.cacheSprite[6].myWidth / 2),
					(Client.frameHeight / 2) - (Client.cacheSprite[6].myHeight / 2));
			Client.cacheSprite[Client.frameMode == Client.ScreenMode.FIXED
					|| client.mouseInRegion(centerX - 83, centerY + 160, centerX - 30, centerY + 197) ? 1 : 0]
							.drawSprite(centerX - 83, centerY + 161);// Fixed
			Client.cacheSprite[Client.frameMode == Client.ScreenMode.RESIZABLE
					|| client.mouseInRegion(centerX - 23, centerY + 160, centerX + 30, centerY + 197) ? 3 : 2]
							.drawSprite(centerX - 23, centerY + 161);// Resizable
			Client.cacheSprite[Client.frameMode == Client.ScreenMode.FULLSCREEN
					|| client.mouseInRegion(centerX + 40, centerY + 160, centerX + 90, centerY + 197) ? 5 : 4]
							.drawSprite(centerX + 37, centerY + 161);// Fullscreen
			...
			Client.smallText.method389(true, centerX + 75, 0xB0AFAB, "Client version " + ClientConstants.CLIENT_VERSION,
					centerY + 100);
			Client.smallText.method389(true, centerX - 115, 0xB0AFAB, "Remember me?", centerY + 16);
			Client.smallText.method389(true, centerX + 40,
					client.mouseInRegion(centerX + 40, centerY + 7, centerX + 127, centerY + 17) ? 0xFFFFFF : 0xB0AFAB,
					"Forgot password?", centerY + 16);
			Client.smallText.method389(true, centerX - 102, 0xB0AFAB,
					TextClass.capitalize(Client.myUsername)
							+ ((client.loginScreenCursorPos == 0) & (Client.loopCycle % 40 < 20) ? "|" : ""),
					centerY - 63);// Username
									// field
			...
			if (client.mouseInRegion(centerX - 138, centerY - 37, centerX - 112, centerY - 11)) {
				Client.smallText.method389(true, centerX - 102, 0xB0AFAB,
						Client.myPassword
								+ ((client.loginScreenCursorPos == 1) & (Client.loopCycle % 40 < 20) ? "|" : ""),
						centerY - 17);
			} else {
				Client.smallText.method389(true, centerX - 102, 0xB0AFAB,
						TextClass.passwordAsterisks(Client.myPassword)
								+ ((client.loginScreenCursorPos == 1) & (Client.loopCycle % 40 < 20) ? "|" : ""),
						centerY - 17);// Password
			} // field
		...
		if (client.loginScreenState == 1) {
			Client.cacheSprite[421].drawARGBSprite((Client.frameWidth / 2) - (Client.cacheSprite[421].myWidth / 2),
					(Client.frameHeight / 2) - (Client.cacheSprite[421].myHeight / 2));
			if (client.mouseInRegion(centerX - 373, centerY + 203, centerX - 297, centerY + 240)) {// World Selection
				client.drawTooltip(client.mouseX + 11, client.mouseY - 20, "World Selection");
				Client.cacheSprite[420].drawSprite(centerX - 373, centerY + 204);
			} else {
		...
		// Client.boldText.method382(0xFFFF00, 100, "MouseX: " + (client.mouseX -
		// (Client.frameWidth / 2)) + " Mouse Y: " + (client.mouseY -
		// (Client.frameHeight / 2)), 100, true);
	...
	private String world[] = { "Ataratix Economy - Main world", "Ataratix PK - Coming soon",
			"Development World - Developers only", "Staff World - Staff member access only" };
			...
			for (int index = 0; index < 3; index++, y += 47) {
				...
				if (account != null) {
					...
					Client.newRegularFont.drawCenteredString(
							(rights != -1 ? "<img=" + rights + "> " : "") + TextClass.capitalize(account.username),
							centerX + 260, y + 35,
							(client.mouseInRegion(centerX + 212, y + 21, centerX + 312, y + 40)) ? 0xFFFFFF : 0xB0AFAB,
							0);
				}
				if (account.uses != 0 && client.mouseX >= centerX + 192 && client.mouseX <= centerX + 212
						&& client.mouseY >= y + 21 && client.mouseY <= y + 40) {
					client.drawTooltip(client.mouseX + 11, client.mouseY - 20, "Account uses: " + account.uses);
				}
				Client.newRegularFont.drawCenteredString("Clear Account List", centerX + 260, centerY - 57,
						(client.mouseInRegion(centerX + 210, centerY - 67, centerX + 311, centerY - 57)) ? 0xFFFFFF
								: 0xB0AFAB,
						0);
			}
		}
	...
	public void processLoginScreen() {
		...
		if (client.loginScreenState == 0) {
			if (client.clickMode3 == 1
					&& client.clickInRegion(centerX - 83, centerY + 160, centerX - 30, centerY + 197))// Fixed Mode
				Client.frameMode(Client.ScreenMode.FIXED);
			if (client.clickMode3 == 1
					&& client.clickInRegion(centerX - 23, centerY + 160, centerX + 30, centerY + 197))// Resizable Mode
				Client.frameMode(Client.ScreenMode.RESIZABLE);
			if (client.clickMode3 == 1
					&& client.clickInRegion(centerX + 40, centerY + 160, centerX + 90, centerY + 197))// Fullscreen Mode
				Client.frameMode(Client.ScreenMode.FULLSCREEN);
			if (client.clickMode3 == 1
					&& client.clickInRegion(centerX - 108, centerY - 82, centerX + 143, centerY - 55))// Username
			...
			if (client.clickMode3 == 1
					&& client.clickInRegion(centerX + 40, centerY + 7, centerX + 127, centerY + 17)) {// Account
																										// Recovery
				client.launchURL("http://www.vencillio.com/forum/31-account-recovery/");
			}
			if (client.clickMode3 == 1
					&& client.clickInRegion(centerX - 373, centerY + 203, centerX - 297, centerY + 240)) {// World
																											// Selection
			...
			if (client.clickMode3 == 1
					&& client.clickInRegion(centerX - 64, centerY + 34, centerX + 69, centerY + 77)) {// Login
	...
	private String[] announcements = { "Welcome to Ataratix!", "Developers are: Daniel, Chex, Seven & Zion.",
			"Vote every 12 hours for great rewards!", "Want to support Ataratix? Purchase membership!" };
	...
	public void displayAnnounements() {
		...
		TextDrawingArea.drawAlphaGradient(0, 0, Client.frameWidth, 25, 0xEBAB36, 0x997309,
				205 - (int) (50 * Math.sin(announcementFade / 20.0)));
