...
import com.atarx.rs2.content.Yelling;
import com.atarx.rs2.content.dialogue.DialogueManager;
import com.atarx.rs2.content.dialogue.OptionDialogue;
import com.atarx.rs2.content.dialogue.impl.ChangePasswordDialogue;
import com.atarx.rs2.content.interfaces.InterfaceHandler;
import com.atarx.rs2.content.interfaces.impl.CommandInterface;
...
public class PlayerCommand implements Command {
	...
	public boolean handleCommand(Player player, CommandParser parser) throws Exception {
		...
		case "commandlist":
		case "commandslist":
			player.send(new SendString("ataratix Command List", 8144));
		...
		/*case "answer":
			if (parser.hasNext()) {
				...
				ataratixBot.answer(player, answer.trim());
			}
			return true;
		...
		case "forums":
		case "website":
			player.send(new SendString("http://www.ataratix.com/", 12000));
			...
			if (parser.hasNext()) {
				try {
					...
					for (int i = 0; i < ataratixConstants.BAD_STRINGS.length; i++) {
						if (message.contains(ataratixConstants.BAD_STRINGS[i])) {
							player.send(new SendMessage("You may not use that in your title!"));
							return true;
						}
					}

					for (int i = 0; i < ataratixConstants.BAD_TITLES.length; i++) {
						if (message.contains(ataratixConstants.BAD_TITLES[i])) {
							player.send(new SendMessage("You may not use that in your title!"));
							return true;
						}
					}
