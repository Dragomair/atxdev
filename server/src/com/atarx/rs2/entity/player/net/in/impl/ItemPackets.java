...
import com.atarx.rs2.content.skill.herblore.SuperCombatPotion;
import com.atarx.rs2.content.skill.hunter.Impling.ImplingRewards;
...
import com.atarx.rs2.content.skill.magic.TabCreation;
...
import com.atarx.rs2.content.skill.smithing.SmithingTask;
...
import com.atarx.rs2.entity.item.ItemCreating;
import com.atarx.rs2.entity.mob.impl.Zulrah;
...
public class ItemPackets extends IncomingPacket {
	...
	public void handle(Player player, StreamBuffer.InBuffer in, int opcode, int length) {
		...
		case 145:
			...
			case 4393:
				if (player.getInterfaceManager().main == 48500) {
					...
					player.getUpdateFlags().sendForceMessage(Utility.randomElement(ataratixConstants.ITEM_IDENTIFICATION_MESSAGES).replaceAll("/s/", "" + aName));
				}
				break;
		...
		case 87:
			...
			for (int index = 0; index < ataratixConstants.ITEM_DISMANTLE_DATA.length; index++) {
				if (itemId == ataratixConstants.ITEM_DISMANTLE_DATA[index][0]) {
					player.getInventory().remove(itemId, 1);
					player.getInventory().addOrCreateGroundItem(ataratixConstants.ITEM_DISMANTLE_DATA[index][1], 1, true);
					player.getInventory().addOrCreateGroundItem(ataratixConstants.ITEM_DISMANTLE_DATA[index][1], 1, true);
