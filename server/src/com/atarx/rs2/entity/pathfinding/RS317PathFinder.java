package com.atarx.rs2.entity.pathfinding;

import java.util.ArrayList;
import java.util.List;

import com.atarx.core.cache.map.Region;
import com.atarx.core.util.Benchmarker;
...
public class RS317PathFinder {
	...
	public static void findRoute(Player c, int destX, int destY,
			boolean moveNear, int xLength, int yLength) {
		Benchmarker.start();
		if (destX == c.getLocation().getLocalX()
				&& destY == c.getLocation().getLocalY() && !moveNear) {
			c.getClient().queueOutgoingPacket(
					new SendMessage("ERROR WITH PATHFINDING"));
			return;
		}
		...
		while (tail != tileQueueX.size() && tileQueueX.size() < pathLength) {
			...
			if (curX > 0
					&& curY > 0
					&& via[curX - 1][curY - 1] == 0
					&& (getClip(curAbsX - 1, curAbsY - 1, c.getLocation()
							.getZ()) & 0x128010e) == 0
					&& (getClip(curAbsX - 1, curAbsY, c.getLocation().getZ()) & 0x1280108) == 0
					&& (getClip(curAbsX, curAbsY - 1, c.getLocation().getZ()) & 0x1280102) == 0) {
				tileQueueX.add(curX - 1);
