package com.atarx.rs2.entity.item;
...
public class ItemDegrading {
	...
	public static final int[][] WEAPON_DEGRADING = { { 13899, 13901 }, { 13902, 13904 }, { 13901, 13901 }, { 13904, 13904 }, { 18349, 18349 }, { 18351, 18351 }, { 18353, 18353 } };

	public static final int[][] EQUIPMENT_DEGRADING = { { 13858, 13860 }, { 13861, 13863 }, { 13864, 13866 }, { 13870, 13872 }, { 13873, 13875 }, { 13876, 13878 }, { 13884, 13886 }, { 13887, 13889 }, { 13890, 13892 }, { 13893, 13895 }, { 13896, 13898 }, { 13860, 13860 }, { 13863, 13863 }, { 13866, 13866 }, { 13872, 13872 }, { 13875, 13875 }, { 13878, 13878 }, { 13886, 13886 }, { 13889, 13889 }, { 13892, 13892 }, { 13895, 13895 }, { 13898, 13898 } };
				...
				if (item != null) {
					...
					if (ref == null) {
						...
						if (index != -1) {
							if (item.getId() != EQUIPMENT_DEGRADING[index][1]) {
								p.getClient().queueOutgoingPacket(new SendMessage("Your " + item.getDefinition().getName() + " has degraded."));
							}
					...
					if (ref != null) {
						...
						if (isFullDegraded(ref)) {
							p.getClient().queueOutgoingPacket(new SendMessage("Your " + item.getDefinition().getName() + " has degraded to dust."));
		...
		if (weapon != null) {
			...
			if (ref == null) {
				...
				if (index != -1) {
					if (weapon.getId() != WEAPON_DEGRADING[index][1]) {
						p.getClient().queueOutgoingPacket(new SendMessage("Your " + weapon.getDefinition().getName() + " has degraded."));
					}
			...
			if (ref != null) {
				...
				if (isFullDegraded(ref)) {
					...
					if (lastStage == -1) {
						p.getClient().queueOutgoingPacket(new SendMessage("Your " + weapon.getDefinition().getName() + " has degraded to dust."));
						p.getEquipment().getItems()[3] = null;
					} else {
						p.getClient().queueOutgoingPacket(new SendMessage("Your " + weapon.getDefinition().getName() + " is now fully degraded and needs repaired."));
