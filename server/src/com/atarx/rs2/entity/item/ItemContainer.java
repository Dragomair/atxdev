...
public abstract class ItemContainer {
		...
		for (int i = 0; i < size; i++) {
			if (items[i] == null) {
				...
				if ((items[i].getAmount() <= 0) && (!allowZero(items[i].getId()))) {
					System.err.println("ZERO AMOUNT ON ITEM");
					return am;
				}
	...
	public void addItems(Item... item) {
		Arrays.stream(item).forEach(this::add);
	}
	...
	public boolean hasSpaceOnRemove(Item add, Item...remove) {
		Item[] itemsCopy = new Item[items.length];
	...
	public boolean isStackable(int id) {
		return (type == ContainerTypes.ALWAYS_STACK) || ((Item.getDefinition(id) != null) && (Item.getDefinition(id).isStackable()) && (type == ContainerTypes.STACK));
	}
	 ...
	 */
	public boolean hasItemId(Item item) {
		for (Item i : items) {
			if (i == null)
				continue;
	...
	public void swap(int to, int from, boolean update) {
