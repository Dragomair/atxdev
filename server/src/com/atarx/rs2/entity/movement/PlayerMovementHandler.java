package com.atarx.rs2.entity.movement;
...
import com.atarx.rs2.entity.player.net.out.impl.SendMultiInterface;
...
public class PlayerMovementHandler extends MovementHandler {
	...
	public void process() {
		...
		if ((walkPoint != null) && (walkPoint.getDirection() != -1)) {
			...
			if ((!forceMove) && (!ataratixConstants.WALK_CHECK) && (!Region.getRegion(player.getLocation()).canMove(player.getLocation(), walkPoint.getDirection()))) {
				reset();
				return;
			}

			player.getMovementHandler().getLastLocation().setAs(player.getLocation());
			player.getLocation().move(com.atarx.rs2.GameConstants.DIR[walkPoint.getDirection()][0], com.atarx.rs2.GameConstants.DIR[walkPoint.getDirection()][1]);
		...
		if (player.getRunEnergy().isRunning()) {
			if (player.getRunEnergy().canRun()) {
				...
				if ((runPoint != null) && (runPoint.getDirection() != -1)) {
					if ((!forceMove) && (!ataratixConstants.WALK_CHECK) && (!Region.getRegion(player.getLocation()).canMove(player.getLocation(), runPoint.getDirection()))) {
					...
					player.getMovementHandler().getLastLocation().setAs(player.getLocation());
					player.getLocation().move(com.atarx.rs2.GameConstants.DIR[runPoint.getDirection()][0], com.atarx.rs2.GameConstants.DIR[runPoint.getDirection()][1]);
