package com.atarx.rs2.entity.following;
...
import com.atarx.rs2.content.combat.Combat;
import com.atarx.rs2.entity.Location;
import com.atarx.rs2.entity.mob.Mob;
import com.atarx.rs2.entity.pathfinding.SimplePathWalker;
...
public class MobFollowing extends Following {
	...
	public void findPath(Location location) {
		if ((mob.isLockFollow()) && (Utility.getManhattanDistance(mob.getLocation(), mob.getOwner().getLocation()) > 10)) {
			onCannotReach();
			...
			if (mob.getAttributes().get("nextfollowdir") == null) {
				...
				if (l != null) {
					...
					if (mob.getMovementHandler().getPrimaryDirection() != -1)
						mob.getAttributes().set("nextfollowdir", Integer.valueOf(mob.getMovementHandler().getPrimaryDirection()));
				}
			} else {
				int dir = mob.getAttributes().getInt("nextfollowdir");
				SimplePathWalker.walkToNextTile(mob, new Location(mob.getX() + GameConstants.DIR[dir][0], mob.getY() + GameConstants.DIR[dir][1], mob.getZ()));
			}
		} else {
			if (mob.getAttributes().get("nextfollowdir") != null) {
	...
	public boolean pause() {
		if (GameConstants.withinBlock(following.getX(), following.getY(), following.getSize(), mob.getX(), mob.getY(), mob.getSize())) {
			return false;
		}
