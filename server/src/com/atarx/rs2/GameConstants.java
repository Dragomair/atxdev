...
public class GameConstants {
	 ...
	 */
	public static final int[][][] SIZES = { { { 0, 0 } }, { { 0, 0 } }, // 1
			{ { 0, 1 }, { 1, 0 }, { 1, 1 } }, // 2
			...
			for (int k = 0; k < size - 1; k++) {
				...
				for (int j = 0; j < size - 1; j++) {
					...
					if (virtual == null) {
						...
						if ((!r.canMove(x3, y3, z, 1)) || (!r.canMove(x3, y3, z, 4)) || (!r.canMove(x3, y3, z, 2)) || (r.isNpcOnTile(x3 + DIR[1][0], y3 + DIR[1][1], z)) || (r.isNpcOnTile(x3 + DIR[4][0], y3 + DIR[4][1], z)) || (r.isNpcOnTile(x3 + DIR[2][0], y3 + DIR[2][1], z))) {
							continue main;
						}

					} else {

						if ((!r.canMove(x3, y3, z, 1)) || (!r.canMove(x3, y3, z, 4)) || (!r.canMove(x3, y3, z, 2)) || (virtual.isMobOnTile(x3 + DIR[1][0], y3 + DIR[1][1], z)) || (virtual.isMobOnTile(x3 + DIR[4][0], y3 + DIR[4][1], z)) || (virtual.isMobOnTile(x3 + DIR[2][0], y3 + DIR[2][1], z))) {
							continue main;
						}
	...
	public static Location[] getEdges(int x, int y, int size) {
		...
		size /= 2;
		return new Location[] { new Location(x - size, y + size), new Location(x, y + size), new Location(x + size, y + size), new Location(x - size, y), new Location(x, y), new Location(x + size, y), new Location(x - size, y - size), new Location(x, y - size), new Location(x + size, y - size) };
	}
