...
public class MembershipBonds {
	...
	public static boolean handle(Player player, int itemId) {
		...
		World.sendGlobalMessage("</col>[ @dre@ataratix </col>] @dre@" + player.determineIcon(player) + " " + Utility.formatPlayerName(player.getUsername()) + "</col> has just reedemed a @dre@" + Utility.format(data.getCredits()) + "</col> credit voucher!");
