package com.atarx.rs2.content.skill.melee;
...
public class BarrowsSpecials {
	...
	public static void doAhrimEffect(Player player, Entity attack, int damage) {
		...
		if ((damage > 0) && (Utility.randomNumber(4) == 0)) {
			...
			if (!attack.isNpc()) {
				Player p2 = com.atarx.rs2.entity.World.getPlayers()[attack.getIndex()];
	...
	public static void doToragEffect(Player player, Entity attack) {
		...
		if (!attack.isNpc()) {
			Player p = com.atarx.rs2.entity.World.getPlayers()[attack.getIndex()];
