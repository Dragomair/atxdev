package com.atarx.rs2.content.skill.farming;
...
public class Compost {
	...
	public void closeCompostBin(final int index) {
		...
		player.getUpdateFlags().sendAnimation(new Animation(835));
		...
		TaskQueue.queue(new Task(player, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
			@Override
			public void execute() {
				player.send(new SendMessage("You close the compost bin, and its content start to rot."));
	...
	public void openCompostBin(final int index) {
		...
		if (Farming.getMinutesCounter(player) - compostBinsTimer[index] >= timerRequired) {
			...
			player.getUpdateFlags().sendAnimation(new Animation(834));
			...
			player.send(new SendMessage("The compost bin is still rotting. I should wait until it is complete."));
		}
	}
	...
	public void fillCompostBin(int x, int y, final int organicItemUsed) {
		...
		if (incrementFactor == 0) {
			player.send(new SendMessage("You need to put organic items into the compost bin in order to make compost."));
			return;
		}
		...
		TaskQueue.queue(new Task(player, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
			@Override
			public void execute() {
				if (!player.getInventory().hasItemId(organicItemUsed) || organicItemAdded[index] == 15) {
					stop();
					return;
				}
				organicItemAdded[index]++;
				player.getUpdateFlags().sendAnimation(new Animation(832));
				player.getInventory().remove(organicItemUsed, 1);
	...
	public void retrieveCompost(final int index) {
		...
		TaskQueue.queue(new Task(player, 2, false, StackType.NEVER_STACK, BreakType.NEVER, TaskIdentifier.FARMING) {
			@Override
			public void execute() {
				if (!player.getInventory().hasItemId(BUCKET) && compostBins[index] != 350 || organicItemAdded[index] == 0) {
					stop();
					return;
				}
				player.getSkill().addExperience(Skills.FARMING, finalItem == COMPOST ? COMPOST_EXP_RETRIEVE : finalItem == SUPER_COMPOST ? SUPER_COMPOST_EXP_RETRIEVE : ROTTEN_TOMATOES_EXP_RETRIEVE);
				if (compostBins[index] != 350) {
					player.getInventory().remove(BUCKET, 1);
				}
	...
	public boolean handleItemOnObject(int itemUsed, int objectId, int objectX, int objectY) {
		...
		case 7827:
			if (itemUsed == BUCKET) {
				...
				player.send(new SendMessage("You might need some buckets to gather the compost."));
			}
			return true;
