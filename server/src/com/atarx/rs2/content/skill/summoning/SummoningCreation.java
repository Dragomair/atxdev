...
public class SummoningCreation {
	public static boolean create(Player p, int button) {
		if (((button >= 155031) && (button <= 156006))
				|| ((button >= 156000) && (button <= 156006))) {
			...
			if ((index >= 0) && (index < Pouch.values().length)) {
				...
				if (pouch != null) {
					if (!p.getInventory().hasSpaceFor(
							new Item(pouch.pouchId, 1))) {
						p.getClient()
								.queueOutgoingPacket(
										new SendMessage(
												"You must free some inventory space to do this."));
						return true;
					}
					...
					if (!ataratixConstants.DEV_MODE) {
						...
						if (!p.getInventory().hasItemAmount(new Item(12525, 1))) {
							p.getClient().queueOutgoingPacket(
									new SendMessage(
											"You do not have an empty pouch."));
							return true;
						}

						if (!p.getInventory().hasItemAmount(
								new Item(18016, shards))) {
							p.getClient()
									.queueOutgoingPacket(
											new SendMessage(
													"You do not have enough shards to create this."));
							return true;
						}

						if (!p.getInventory().hasItemAmount(
								new Item(ingredient, 1))) {
							String name = Item.getDefinition(ingredient)
									.getName();
							p.getClient().queueOutgoingPacket(
									new SendMessage("You need "
											+ Utility.getAOrAn(name) + " " + name
											+ " to create this."));
							return true;
						}
