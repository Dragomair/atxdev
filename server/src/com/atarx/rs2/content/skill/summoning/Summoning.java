...
import com.atarx.rs2.content.dialogue.impl.ConfirmDialogue;
...
import com.atarx.rs2.entity.player.net.out.impl.SendModelAnimation;
import com.atarx.rs2.entity.player.net.out.impl.SendNPCDialogueHead;
...
public class Summoning {
	...
	public void doSpecial() {
		if (familiar != null) {
			...
			if (spec != null) {
				...
				if ((spec.getSpecialType() == FamiliarSpecial.SpecialType.COMBAT) && (familiar.getCombat().getAttacking() == null)) {
					p.getClient().queueOutgoingPacket(new SendMessage("You must be attacking something to use this special move!"));
					return;
				}
				...
				if (special < spec.getAmount()) {
					p.getClient().queueOutgoingPacket(new SendMessage("You do not have enough special points to perform that attack."));
					return;
				}
	...
	public boolean summon(int id) {
		...
		if (f != null) {
			if ((!ataratixConstants.DEV_MODE) && (!SummoningConstants.isAllowed(f))) {
				p.getClient().queueOutgoingPacket(new SendMessage("This familiar is not yet functional."));
				return true;
			}

			if (p.getMaxLevels()[21] < f.levelRequired) {
				p.getClient().queueOutgoingPacket(new SendMessage("You need a Summoning level of " + f.levelRequired + " to summon this mob."));
				return true;
			}
