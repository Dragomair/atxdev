package com.atarx.rs2.content.minigames.fightpits;
...
public class FightPits {
	...
	public static final boolean clickObject(Player p, int id) {
		switch (id) {
		case 9369:
			if ((!p.getController().equals(ControllerManager.FIGHT_PITS_WAITING_CONTROLLER)) && (p.getController().equals(ControllerManager.DEFAULT_CONTROLLER))) {
		...
		case 9368:
			if (!p.getController().equals(ControllerManager.FIGHT_PITS_CONTROLLER)) {
				p.getClient().queueOutgoingPacket(new SendMessage("The game has not started yet. You will moved inside once it does."));
			} else {
				inGame.remove(p);
				waiting.add(p);
	...
	public static final Location getRandomizedStartLocation() {
		int x = 2383 + Utility.randomNumber(30);
		int y = 5133 + Utility.randomNumber(27);

		while (Region.getStaticClip(x, y, 0) != 0) {
			x = 2383 + Utility.randomNumber(30);
			y = 5133 + Utility.randomNumber(27);
		}
	...
	public static final void updateInterface(Player p) {
		p.getClient().queueOutgoingPacket(new SendWalkableInterface(2804));
		p.getClient().queueOutgoingPacket(new SendString("Current Champion: " + champion, FightPitsConstants.FIGHT_PITS_INTERFACE_STRINGS[0]));
		if (endTime == 0) {
			p.getClient().queueOutgoingPacket(new SendString("Time until next game: " + startTime / 2, FightPitsConstants.FIGHT_PITS_INTERFACE_STRINGS[1]));
			p.getClient().queueOutgoingPacket(new SendString("Players waiting: " + (waiting.size() + inGame.size()), FightPitsConstants.FIGHT_PITS_INTERFACE_STRINGS[2]));
		} else {
			p.getClient().queueOutgoingPacket(new SendString("Time until game ends: " + endTime / 2, FightPitsConstants.FIGHT_PITS_INTERFACE_STRINGS[1]));
			p.getClient().queueOutgoingPacket(new SendString("Foes Remaining: " + inGame.size(), FightPitsConstants.FIGHT_PITS_INTERFACE_STRINGS[2]));
		}
	}
