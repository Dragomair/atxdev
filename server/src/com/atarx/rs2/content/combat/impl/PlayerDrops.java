...
import com.atarx.rs2.content.wilderness.BountyEmblems;
import com.atarx.rs2.content.wilderness.PlayerKilling;
	...
	public static final void dropItemsOnDeath(Player player) {
		...
		while ((k = items.poll()) != null) {
			...
			for (int index = 0; index < ataratixConstants.ITEM_DISMANTLE_DATA.length; index++) {
				if (k.getId() == ataratixConstants.ITEM_DISMANTLE_DATA[index][0]) {
					k.setId(ataratixConstants.ITEM_DISMANTLE_DATA[index][1]);
				}
			}
