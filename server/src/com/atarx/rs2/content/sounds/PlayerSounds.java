...
import com.atarx.rs2.entity.Sound;
...
public class PlayerSounds {
	...
	public static void sendBlockOrHitSound(Player player, boolean hit) {
		...
		if ((shield != null) && (sounds[shield.getId()] != null)) {
			player.getClient().queueOutgoingPacket(new SendSound(sounds[shield.getId()][com.atarx.core.util.Utility.randomNumber(sounds[shield.getId()].length)].id, 0, 0));
		}
	...
	public static void sendSoundForId(Player player, boolean spec, int id) {
		if (sounds[id] != null)
			if (spec)
				...
				player.getClient().queueOutgoingPacket(new SendSound(sounds[id][player.getEquipment().getAttackStyle().ordinal()]));
	}
}
