...
public class MobSounds {
	...
	public static void declare() {
		for (int i = 0; i < sounds.length; i++) {
			...
			if ((def != null) && (def.getName() != null)) {
				...
				if ((s.contains("man")) || (s.contains("mugger"))
						|| (s.contains("mage")) || (s.contains("wizard")))
				...
				else if ((s.contains("hellhound"))
						|| (s.contains("hell hound")) || (s.contains("wolf")))
	...
	public static void sendAttackSound(Player p, int id, CombatTypes type,
			boolean hit) {
		...
		if (type == CombatTypes.MELEE) {
			...
			p.getClient().queueOutgoingPacket(
					new SendSound(sounds[id][0], 10, 20));
			...
			if ((sounds[id] != null) && (sounds[id][1] > 0))
				p.getClient().queueOutgoingPacket(
						new SendSound(sounds[id][1], 10, 20));
		} else if ((type == CombatTypes.MAGIC) && (sounds[id] != null)
				&& (sounds[id][2] > 0)) {
			...
			if (!hit)
				...
				p.getClient().queueOutgoingPacket(
						new SendSound(sounds[id][2], 10, 20));
		}
	}
	...
	public static void sendBlockSound(Player p, int id) {
		...
		if ((sounds[id] != null) && (sounds[id][3] > 0)) {
			p.getClient().queueOutgoingPacket(
					new SendSound(sounds[id][3], 10, 10));
		}
	}
	...
	public static void sendDeathSound(Player p, int id) {
		...
		if ((sounds[id] != null) && (sounds[id][4] > 0)) {
			p.getClient().queueOutgoingPacket(
					new SendSound(sounds[id][4], 10, 10));
		}
	}
}
