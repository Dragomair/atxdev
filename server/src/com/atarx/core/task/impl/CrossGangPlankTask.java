...
public abstract class CrossGangPlankTask extends ForceMovementTask {
	...
	public CrossGangPlankTask(Player player, Location dest, Controller to,
			boolean on) {
	...
	public void execute() {
		if (++steps == 2) {
			player.teleport(new Location(player.getLocation().getX(), player
					.getLocation().getY(), on ? 1 : 0));
		} else if (steps > 6) {// just in case
			player.getMovementHandler().reset();
			player.teleport(dest);
			onDestination();
			player.setController(to);
			player.getUpdateFlags().sendFaceToDirection(
					player.getLocation().getX() - xMod,
		...
		if (player.getLocation().getX() + xMod == dest.getX()
				&& player.getLocation().getY() + yMod == dest.getY()) {
			onDestination();
			player.setController(to);
			player.getUpdateFlags().sendFaceToDirection(
					player.getLocation().getX() - xMod,
