...
import com.atarx.core.cache.ByteStream;
...
public class MapLoading {
	...
	public static void addObject(boolean beforeLoad, int objectId, int x, int y, int height, int type, int direction) {
		...
		/*
		 * if (def.name != null && def.name.contains("Flax")) { def.unwalkable =
		 * false; }
		 * 
		 ...
		 * 
		 * if (def.unshootable) { addProjectileClipping(beforeLoad, x, y,
		 * height, 0x200000); } } } else if (type >= 9) { if (def.clipped()) {
		 * addClippingForSolidObject(beforeLoad, x, y, height, xLength, yLength,
		 * def.unshootable); } } else if (type >= 0 && type <= 3) { if
		 * (def.clipped() || def.hasActions) {
		 * addClippingForVariableObject(beforeLoad, x, y, height, type,
		 * direction, def.unshootable); }
		 * 
		 * if (def.unshootable) {
		 * addProjectileClippingForVariableObject(beforeLoad, x, y, height,
		 * type, direction, true); } }
		 */
		...
		if (def.hasActions()) {
			(beforeLoad ? Region.getUnsortedRegion(x, y) : Region.getRegion(x, y)).addObject(new RSObject(x, y, height, objectId, type, direction));
			if (beforeLoad) {
				if (def.actions != null && def.name != null) {
					if (objectId == 1586 || objectId == 23156 || objectId == 15516 || def.name.toLowerCase().contains("door") && !def.name.toLowerCase().contains("trapdoor") || def.name.toLowerCase().contains("gate")) {
	...
	public static void removeObject(int objectId, int x, int y, int height, int type, int direction) {
		...
		if (type == 22) {
			...
			if (def.aBoolean779) {
				removeClippingForVariableObject(x, y, height, type,
						direction, /*
									 * def . isSolid ( )
									 */
						def.aBoolean757);
	...
	public static void removeClippingForVariableObject(int x, int y, int height, int type, int direction, boolean flag) {
		 ...
		 * addProjectileClipping(before, x - 1, y, height, -8); } if (direction
		 * == 1) { addProjectileClipping(before, x, y, height, -2);
		 * addProjectileClipping(before, x, y + 1, height, -32); } if (direction
		 * == 2) { addProjectileClipping(before, x, y, height, -8);
		 * addProjectileClipping(before, x + 1, y, height, -128); } if
		 * (direction == 3) { addProjectileClipping(before, x, y, height, -32);
		 * addProjectileClipping(before, x, y - 1, height, -2); } } if (type ==
		 * 1 || type == 3) { if (direction == 0) { addProjectileClipping(before,
		 * x, y, height, -1); addProjectileClipping(before, x - 1, y + 1,
		 * height, -16); } if (direction == 1) { addProjectileClipping(before,
		 * x, y, height, -4); addProjectileClipping(before, x + 1, y + 1,
		 * height, -64); } if (direction == 2) { addProjectileClipping(before,
		 * x, y, height, -16); addProjectileClipping(before, x + 1, y - 1,
		 * height, -1); } if (direction == 3) { addProjectileClipping(before, x,
		 * y, height, -64); addProjectileClipping(before, x - 1, y - 1, height,
		 * -4); } } if (type == 2) { if (direction == 0) {
		 * addProjectileClipping(before, x, y, height, -130);
		 * addProjectileClipping(before, x - 1, y, height, -8);
		 * addProjectileClipping(before, x, y + 1, height, -32); } if (direction
		 * == 1) { addProjectileClipping(before, x, y, height, -10);
		 * addProjectileClipping(before, x, y + 1, height, -32);
		 * addProjectileClipping(before, x + 1, y, height, -128); } if
		 * (direction == 2) { addProjectileClipping(before, x, y, height, -40);
		 * addProjectileClipping(before, x + 1, y, height, -128);
		 * addProjectileClipping(before, x, y - 1, height, -2); } if (direction
		 * == 3) { addProjectileClipping(before, x, y, height, -160);
	...
	public static void addClippingForVariableObject(boolean before, int x, int y, int height, int type, int direction, boolean flag) {
		 ...
		 * addClippingAlternate(before, 4096, x - 1, y, height); } if (direction
		 * == 1) { addClippingAlternate(before, 1024, x, y, height);
		 * addClippingAlternate(before, 16384, x, y + 1, height); } if
		 * (direction == 2) { addClippingAlternate(before, 4096, x, y, height);
		 * addClippingAlternate(before, 0x10000, x + 1, y, height); } if
		 * (direction == 3) { addClippingAlternate(before, 16384, x, y, height);
		 * addClippingAlternate(before, 1024, x, y - 1, height); } } if (type ==
		 * 1 || type == 3) { if (direction == 0) { addClippingAlternate(before,
		 * 512, x, y, height); addClippingAlternate(before, 8192, x - 1, y + 1,
		 * height); } if (direction == 1) { addClippingAlternate(before, 2048,
		 * x, y, height); addClippingAlternate(before, 32768, x + 1, y + 1,
		 * height); } if (direction == 2) { addClippingAlternate(before, 8192,
		 * x, y, height); addClippingAlternate(before, 512, x + 1, y - 1,
		 * height); } if (direction == 3) { addClippingAlternate(before, 32768,
		 * x, y, height); addClippingAlternate(before, 2048, x - 1, y - 1,
		 * height); } } if (type == 2) { if (direction == 0) {
		 * addClippingAlternate(before, 0x10400, x, y, height);
		 * addClippingAlternate(before, 4096, x - 1, y, height);
		 * addClippingAlternate(before, 16384, x, y + 1, height); } if
		 * (direction == 1) { addClippingAlternate(before, 5120, x, y, height);
		 * addClippingAlternate(before, 16384, x, y + 1, height);
		 * addClippingAlternate(before, 0x10000, x + 1, y, height); } if
		 * (direction == 2) { addClippingAlternate(before, 20480, x, y, height);
		 * addClippingAlternate(before, 0x10000, x + 1, y, height);
		 * addClippingAlternate(before, 1024, x, y - 1, height); } if (direction
		 * == 3) { addClippingAlternate(before, 0x14000, x, y, height);
		 * addClippingAlternate(before, 1024, x, y - 1, height);
		 * addClippingAlternate(before, 4096, x - 1, y, height); } } }
		 */

		/*
		 * if (type == 0) { // Normal walls if (direction == 0) { // Wall
		 * tile-west? addClipping(before, x, y, height, 128);
		 * addClipping(before, x - 1, y, height, 8); } else if (direction == 1)
		 * { // Wall tile-north? addClipping(before, x, y, height, 2);
		 * addClipping(before, x, y + 1, height, 32); } else if (direction == 2)
		 * { // Wall tile-east? addClipping(before, x, y, height, 8);
		 * addClipping(before, x + 1, y, height, 128); } else if (direction ==
		 * 3) { // Wall tile-south? addClipping(before, x, y, height, 32);
		 * addClipping(before, x, y - 1, height, 2); } // Diagonal walls } else
		 * if (type == 1 || type == 3) { if (direction == 0) { // Wall
		 * north-west??? addClipping(before, x, y, height, 1);
		 * addClipping(before, x - 1, y, height, 16); } else if (direction == 1)
		 * { // Wall north-east addClipping(before, x, y, height, 4);
		 * addClipping(before, x + 1, y + 1, height, 64); } else if (direction
		 * == 2) { // Wall south-east addClipping(before, x, y, height, 16);
		 * addClipping(before, x + 1, y - 1, height, 1); } else if (direction ==
		 * 3) { // Wall south-west addClipping(before, x, y, height, 64);
		 * addClipping(before, x - 1, y - 1, height, 4); } } else if (type == 2)
		 * { if (direction == 0) { // Blocked north and west addClipping(before,
		 * x, y, height, 130); addClipping(before, x - 1, y, height, 8);
		 * addClipping(before, x, y + 1, height, 32); } else if (direction == 1)
		 * { // Blocked north and east addClipping(before, x, y, height, 10);
		 * addClipping(before, x, y + 1, height, 32); addClipping(before, x + 1,
		 * y, height, 128); } else if (direction == 2) { // Blocked south and
		 * east addClipping(before, x, y, height, 40); addClipping(before, x +
		 * 1, y, height, 128); addClipping(before, x, y - 1, height, 2); } else
		 * if (direction == 3) { // Blocked south and west addClipping(before,
		 * x, y, height, 160); addClipping(before, x, y - 1, height, 2);
		 * addClipping(before, x - 1, y, height, 8); } }
		 */
	}
