...
public class MapConstants {
	...
	public static final RSObject getDoubleDoor(final int id, final int x,
			final int y, final int z, final int face) {
		...
		for (int i = 0; i < 8; i++) {
			...
			if (o != null && (d = ObjectDef.getObjectDef(o.getId())) != null
	...
	public static final void writeAlternateIds() {
		...
		for (int i = 0; i < alts.length; i++) {
			...
			if (ObjectDef.getObjectDef(i) != null
					&& ObjectDef.getObjectDef(i).name != null) {
				if (ObjectDef.getObjectDef(i).name.toLowerCase().contains(
						"door")
						|| ObjectDef.getObjectDef(i).name.toLowerCase()
								.contains("gate")) {
